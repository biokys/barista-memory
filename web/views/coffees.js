...
/** Roast levels and processes as stored; the labels come from the dictionary. */
...
function processLabel(process) {
  return process ? (PROCESSES.includes(process) ? t("coffees.process_of." + process) : process) : null;
}

/**
 * A select over known keys. A value typed before the select existed, or by
 * the MCP, stays as its own option rather than silently changing on save.
 */
function keyedSelect(name, keys, value, labelOf) {
  const options = keys.includes(value) || !value ? keys : [value, ...keys];
  return `<div class="field"><label>${t("coffees." + name)}</label><select name="${name}"><option value="">–</option>${options.map((k) => `<option value="${k}" ${k === value ? "selected" : ""}>${labelOf(k)}</option>`).join("")}</select></div>`;
}
...
function form(c = {}, submitLabel) {
  ...
  return `
    <form class="form" id="cf">
      ...
      <div class="grid cols-3">${input("origin", "text")}${keyedSelect("process", PROCESSES, c.process, processLabel)}${keyedSelect("roast_level", ROAST_LEVELS, c.roast_level, roastLabel)}</div>
  ...
  const row = (c) => `
    <a class="shot-row" href="#/coffees/${c.id}">
      <div class="main">
        ...
        <div class="meta">${[c.origin, processLabel(c.process), roastLabel(c.roast_level)].filter(Boolean).join(" · ") || "&nbsp;"}</div>
      </div>
      <div class="nums num">
...
async function renderDetail(view, id) {
  ...
  view.innerHTML = `
    <div class="row spread">
      <div>
        <h1>${c.name}</h1>
        <p class="muted">${[c.roaster, c.origin, processLabel(c.process), roastLabel(c.roast_level)].filter(Boolean).join(" · ")}</p>
      </div>
      <div class="row">
